...
class ReportService:
    ...
    async def generate_production_report(
        ...
        avg_per_record = total_liters_produced / total_records if total_records > 0 else Decimal("0")
        ...
        retention_percentage = (
            ...
            if total_liters_produced > 0 else Decimal("0")
        )
        ...
        top_producers = []
        for animal_id, total_liters in sorted(animal_totals.items(), key=lambda x: x[1], reverse=True)[:10]:
            ...
            if animal:
                top_producers.append({
                    ...
                    'avg_per_day': total_liters / ((request.date_to - request.date_from).days + 1)
                })
        ...
        subtitle = f"Período: {request.date_from.strftime('%d/%m/%Y')} - {request.date_to.strftime('%d/%m/%Y')}"
        ...
        if period_production_data:
            elements.extend(self.pdf_generator.create_chart_section(
        ...
        if top_producers:
            ...
            elements.extend(self.pdf_generator.create_table_section(
                "Top 10 Animales Productores",
                table_data,
                columns
            ))
    ...
    async def generate_animals_report(
        ...
        # Calculate averages
        for animal_id, perf in animal_performance.items():
        ...
        for animal in animals:
            perf = animal_performance.get(animal.id, {
                'total_liters': Decimal("0"),
                'records_count': 0,
                'avg_per_record': Decimal("0")
            })
        ...
        elements.extend(self.pdf_generator.create_table_section(
            "Rendimiento Individual por Animal",
            table_data,
            columns
        ))
